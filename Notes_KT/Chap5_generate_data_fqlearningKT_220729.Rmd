...
for (idxGroup in 1:nGroup){
  ...
  # この手順よくわかってない
  alphaL <- pmax(pmin(alphaL,1.0),0.01) # 0.01以上1.0以下に収める
  alphaF <- numeric(nSub)               # nSub列の空配列を作る

  alphaF <- alphaL # F-Q learning model
  # alphaF <- 0 # Q-learning model

  # inverted temperature 
  beta <- rnorm(n = nSub, mean = beta.mean[idxGroup],
                sd = beta.sigma[idxGroup])
  beta <- pmax(pmin(beta,10.0),0.01) # 0.01 以上10以下に抑える
  ...
  # 7/29 From Here L.88 at generate_data_fqlearning_single_subject.Rmd
...
pmin(c(20, 10, 4, 0),10.0) # 配列の最大値を10にしてしまう 32とかあったら10に置換
pmax(c(20, 10, 4, 0),10.0) # 配列の最小値を10にしてしまう
